...
public class OrderDAO extends BaseJdbcDAO implements IOrderDAO {
	...
	private final RowMapper<OrderDetailPojo> rowMapperOrderDetail = new RowMapper<OrderDetailPojo>() {
		...
		@Override
		public OrderDetailPojo mapRow(ResultSet rs, int rowNum)
				throws SQLException {
	...
	public int addOrderDetail(List<OrderPojo> orderList, String orderID) {
		...
		for (OrderPojo order : orderList) {
			...
			if (controlExistingOrderDetail(orderID, order.getItemID()) == null) {
				 query = "INSERT INTO oft_order_detail (ORDER_ID,ITEM_ID,ITEM_COUNT) VALUES (:orderID,:itemID,:itemCount)";
			} else {
				 query = "update oft_order_detail set ITEM_COUNT=ITEM_COUNT + :itemCount WHERE ORDER_ID=:orderID and ITEM_ID=:itemID";
			}
	...
	public List<MenuPojo> getOrder(int roomNO) {
		...
		List<MenuPojo> orderDetail = namedParameterJdbcTemplate.query(query,
				parameters, rowMapperMenuDetail);
		return orderDetail;
	}
	...
	public OrderPojo controlExistingOpenOrder(int roomNO)
		...
		List<OrderPojo> order = namedParameterJdbcTemplate.query(query,
				parameters, rowMapperOrder);
	...
	public int controlDeleteCount(int itemID, String orderID)
		...
		List<OrderDetailPojo> orderDetail = namedParameterJdbcTemplate.query(
				query, parameters, rowMapperOrderDetail);
	...
	private String controlExistingOrderDetail(String orderID, int itemID) {
		...
		namedParameters.put("orderID", orderID);
		namedParameters.put("itemID", itemID);
		List<OrderDetailPojo> list = namedParameterJdbcTemplate.query(query,
				namedParameters, rowMapperOrderDetail);
